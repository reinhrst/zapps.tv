(ns tv.zapps.purkinje.zippo-tester
  ...
  (:import java.net.Socket java.util.Date java.io.IOException))
...
(defn -main [& args]
  ...
  (let [zippo-server (nth args 0)
    ...
    (let [sequence (core/fingerprint-sequence-from-url-string url-string)
          _ (doall (take 200 sequence)) ; syncing sequence
          ...
          read-bytes (fn [nr] (let [bs (repeatedly nr #(.read inputstream))]
                                (when (seq (filter #(= -1 %) bs))
      ...
      (.start (Thread. (fn []
                         (try
                           (loop []
                             ...
                             (recur))
                           (catch IOException e (log/error "Connection closed"))))))
      ...
      (doseq [fingerprint (take 256 (drop 200 sequence))]
