...
for channel in channels:
  if channel["active"] and channel["tubby_hostname"] == hostname:
      streaming_command = "echo 'Now streaming'%(name)s' from '%(device)s' to '%(tubby_port)s && cvlc v4l2://%(device)s --v4l2-tuner-frequency=%(frequency)s000  --v4l2-standard PAL --sout='#standard{access=http,mux=ts,dst=*:'%(tubby_port)s'}'" % {x:pipes.quote(str(y)) for x,y in channel.items()}
