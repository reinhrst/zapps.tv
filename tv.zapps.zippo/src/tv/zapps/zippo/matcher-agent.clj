(ns tv.zapps.zippo.matcher-agent
  (:require [clojure.tools.logging :as log])
  (:require [tv.zapps.zippo.uncorrelated-matching-data]))

(def MINIMUM_FINGERPRINTS_FOR_MATCH_SUGGESTION 30)
(def MAXIMUM_LOOKBACK_FRAMES 1000)
(def MAXIMUM_FINGERPRINTS_IN_AGENT 1000)
(def TRUNCATE_TO_FINGERPRINTS_IN_AGENT 128)
(def POSITIVE_CORRELATION_BEFORE_REPORTING 0.94)
(def POSITIVE_CORRELATION_BEFORE_STORING 0.80)
...
(defn- get-certainty [size surroundings diff]
   (let [correlation-cutoff-info (tv.zapps.zippo.uncorrelated-matching-data/get-correlation-cutoff size surroundings)
         certainty-keyword (first
                            (filter
                             #(< diff (% correlation-cutoff-info))
                             [:0.999 :0.995 :0.99 :0.95 :0.90]))]
     (if certainty-keyword
       (-> certainty-keyword name Float/parseFloat)
       0.7)))
...
(defn match-on-location-single-channel [received-data-to-match channel-data channel-id scoring]
  ...
  (let [channel-to-matching-timestamp-correction (:offset scoring)
    ...
    (when (> certainty POSITIVE_CORRELATION_BEFORE_STORING)
      (map->Scoring
...
(defn match-no-history-single-channel [data-to-match, channel-data, channel-id]
  {:pre [(valid-to-match-data? data-to-match)
         (<= (count data-to-match) TRUNCATE_TO_FINGERPRINTS_IN_AGENT)
         (valid-channel-data? channel-data)]
   :post [(vector-of-type? (vector %) Scoring [:size :diff :offset :certainty :channel-id])]}
...
(defn match [data-to-match, channels-data-and-ids, scoring]
  {:pre [(<= (count data-to-match) TRUNCATE_TO_FINGERPRINTS_IN_AGENT)]
  ...
  (map
   (fn [connection-data-atom]
     (let [connection-data @connection-data-atom
           latest-fingerprints (take-last (+ MAXIMUM_LOOKBACK_FRAMES TRUNCATE_TO_FINGERPRINTS_IN_AGENT) (:fingerprints connection-data))
...
(defn- add-data-to-match-to-agent-payload [agent-payload data-to-match]
  {:pre [(valid-to-match-data? data-to-match)
         (valid-to-match-data? (:data-to-match agent-payload))]
   :post [(valid-to-match-data? (:data-to-match %))]}
  (let [combined-data (apply (partial conj (:data-to-match agent-payload)) data-to-match)]
    (assoc agent-payload :data-to-match (if (> (count combined-data) MAXIMUM_FINGERPRINTS_IN_AGENT)
                                          (vec (take-last TRUNCATE_TO_FINGERPRINTS_IN_AGENT combined-data))
                                          combined-data))))

(defrecord AgentPayload
    [self
     scorings
     fingerprints
     output-stream
...
(defn get-connection-agent [output-stream, channels-data-atom]
  (let [my-agent (agent (map->AgentPayload
                         {:scorings []
                          :data-to-match []
                          :output-stream output-stream
                          :channels-data-atom channels-data-atom}))]
    (send-off my-agent (fn [agent-payload] (assoc agent-payload :self my-agent)))))

(defn- match-data-from-scratch-for-agent [agent-payload data-to-match channels-data]
  {:post [(= (class %) AgentPayload)]}
  
(defn- match-data-progressively-for-agent [agent-payload data-to-match channels-data]
  {:post [(= (class %) AgentPayload)]}
  (let [old-scorings (:scorings agent-payload)
        new-scorings (map
                      (fn [scoring]
                        (let [new-scoring (match-on-location-single-channel
                                           data-to-match
                                           (deref (nth (:channel scoring) channels-data))
                                           scoring)]
                          (assoc new-scoring :certainty (get-certainty
                                                         (count data-to-match)
                                                         SURROUNDINGS_FOR_ON_LOCATION_MATCH
                                                         (:diff new-scoring)))))
                      
                      old-scorings)
        alpha (ALPHA (count data-to-match))
        combined-scorings (filter
                           #(> (:certainty %) POSITIVE_CORRELATION_CERTAINTY_KEY)
                           (map
                            (fn [old-scoring new-scoring]
                              (map->Scoring
                               {:channel (:channel old-scoring)
                                :size (+ (:size old-scoring) (:size new-scoring))
                                :diff (+ (:diff old-scoring) (:diff new-scoring))
                                :offset (:offset new-scoring) ; not sure, which one to take.... probably should only take new one if we got enough new data
                                :certainty (+ (* alpha (:certainty old-scoring)) (* (- 1 alpha) (:certainty new-scoring)))}))
                            old-scorings
                            new-scorings))]
                                        ; TODO try the same offset but different channels if no match
    (assoc agent-payload :scorings combined-scorings)))
    
        
(defn- send-scorings [agent-payload]
  )
  

(defn- match-fingerprints-v1 [data-to-match]
  {:pre [(valid-to-match-data? data-to-match)]}
  (fn [agent-payload]
    (let [new-agent-payload (add-data-to-match-to-agent-payload agent-payload data-to-match)
          channels-data (deref (:channels-data-atom agent-payload))]
      (if (seq (:scorings new-agent-payload))
        (let [new-scorings-agent-payload (match-data-progressively-for-agent new-agent-payload data-to-match channels-data)]
          (if (seq (:scorings new-scorings-agent-payload))
            (send-scorings new-scorings-agent-payload)
            (send-off (:self new-scorings-agent-payload) (match-fingerprints-v1 [])))
          new-scorings-agent-payload)
        (let [new-scorings-agent-payload (match-data-from-scratch-for-agent new-agent-payload data-to-match channels-data)]
          (send-scorings new-scorings-agent-payload)
          new-scorings-agent-payload)))))

(def zippo-protocol
  {:match-fingerprints-v1 match-fingerprints-v1
   :3 mat
        (
                                     )))))})
